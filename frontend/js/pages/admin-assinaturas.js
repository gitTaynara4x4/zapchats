...
function formatDate(iso) {
  return !iso || Number.isNaN(new Date(iso).getTime()) ? "-" : new Date(iso).toLocaleDateString("pt-BR");
}
function formatDateTime(iso) {
  return !iso || Number.isNaN(new Date(iso).getTime()) ? "-" : new Date(iso).toLocaleString("pt-BR");
}
...
function renderDashboardView() {
  ...
  let mrr = 0;
  rows.forEach(r => {
    if ((r.subscription_status === "active" || r.subscription_status === "trial") && r.price_monthly) {
      mrr += Number(r.price_monthly);
    }
  });
  ...
  const planos = {};
  rows.forEach(r => {
    const p = r.effective_tier || "FREE";
    planos[p] = (planos[p] || 0) + 1;
  });
...
function statItem(label, current, max) {
  ...
  const colorClass = percent >= 100 ? "danger" : (percent >= 80 ? "warning" : "");
...
function renderDetail(item) {
  ...
  if (els.detailCompanyInfo) {
    els.detailCompanyInfo.textContent = `ID: #${emp.id} • ${emp.nome_adm || "Sem admin"} • ${emp.telefone || "-"} • CNPJ: ${emp.cnpj_cpf || "N/A"}`;
  }
...
async function doLogin() {
  ...
  try {
    const data = await api(`/auth/login`, {
      method: "POST",
      body: JSON.stringify({ password: (els.adminPasswordInput.value || "").trim() })
    });
    ...
    if (els.loginError) els.loginError.textContent = err.message || "Senha inválida.";
  }
}
...
if (els.saveNotesBtn) {
  els.saveNotesBtn.addEventListener("click", async () => {
    ...
    try {
      await api(`/empresas/${state.selectedId}/notes`, {
        method: "PUT",
        body: JSON.stringify({ internal_notes: els.internalNotes.value })
      });
...
if (els.magicLoginBtn) {
  els.magicLoginBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      showToast("Falha no acesso mágico.", true);
    }
  });
}
...
async function applyPlan() {
  ...
  try {
    ...
    await api(`/empresas/${state.selectedId}/apply-plan`, {
      method: "POST",
      body: JSON.stringify({
        assinatura: els.planSelect.value,
...
async function startTrial() {
  if (!state.selectedId) return;
  try {
    await api(`/empresas/${state.selectedId}/start-trial`, {
      method: "POST",
      body: JSON.stringify({
...
async function saveOverrides() {
  ...
  try {
    await api(`/empresas/${state.selectedId}/overrides`, {
      method: "POST",
      body: JSON.stringify({
...
async function saveLoginConfig() {
  if (!state.selectedId) return;
  try {
    await api(`/empresas/${state.selectedId}/login-config`, {
      method: "PUT",
      body: JSON.stringify({ requer_token_login: !!els.requerTokenLogin.checked })
    });
...
if (els.bulkApplyPlanBtn) {
  els.bulkApplyPlanBtn.addEventListener("click", async () => {
    ...
    try {
      await api(`/empresas/bulk-action`, {
        method: "POST",
        body: JSON.stringify({
...
function bindEvents() {
  ...
  if (els.logoutBtn) {
    els.logoutBtn.addEventListener("click", () => {
      ...
      showDashboard();
      if (els.companyList) {
        els.companyList.innerHTML = `<div class="empty-state"><p>Faça login para carregar os clientes.</p></div>`;
      }
