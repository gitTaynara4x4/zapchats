...
${lista}`
  ...
  function ensureDeptItems(){
    cfg.features.auto_messages_departments ||= { enabled:false, welcome:{enabled:false,text:"",start:"08:00",end:"18:00"}, items:{} };
  ...
  function seedDeptItemsDefault(){
    ...
    // Se ainda não tem nada salvo: padrão = todos ligados
    ...
    // Mantém label sincronizado + adiciona dept novo (padrão ligado)
    _deptCache.forEach(d=>{
      const id = String(d.id);
  ...
  function renderDeptPicker(){
    if(!deptList) return;
    if(!cfg){ deptList.innerHTML=''; return; }
  ...
  function validateBeforeSave(kind='auto'){
    ...
    if(kind==='dept' && getSwitch(swDeptHdr)){
      ...
      // precisa ter ao menos 1 departamento selecionado
      if (Array.isArray(_deptCache) && _deptCache.length) {
  ...
  async function loadAll(){
    ...
    renderDeptPicker();updateScheduleVisibility(); updateSaveButtons();
  }
