(function () {
  ...
  const CFG = {
    key: 'dashboard',
    page: 'Dashboard',
    title: 'Entenda seu Dashboard',
    intro: 'Vou te mostrar os principais pontos desta tela para você navegar com mais segurança.',
    robotName: 'Zappy',
    robotSrc: '/frontend/img/help/zc-robot-floating.png',
    storageKey: 'zaps_help_dashboard_robot_tour_seen_v5',
    steps: [
      {
        title: 'Resumo da página',
        say: 'Oi! Vamos começar pelo topo da tela para você se localizar rapidinho.',
        desc: 'Aqui você confirma que está no Dashboard e entende rapidamente o objetivo da tela. Os filtros ficam separados para não misturar navegação com indicadores.',
        icon: 'grid',
        selectors: ['.dash-hero-copy', '.dash-hero h1', '.dashboard-analytics h1']
      },
      {
        title: 'Filtros do período',
        say: 'Agora olha aqui: é nessa área que você troca instância e período.',
        desc: 'Use esta área para escolher a instância de WhatsApp e a data analisada. Ao mudar os filtros, os indicadores e gráficos são atualizados.',
        icon: 'filter',
        selectors: ['.dash-hero-actions', '#instMenuBtn', '.filter', '#filtroData']
      },
      {
        title: 'Indicadores principais',
        say: 'Esses cards são o seu resumo rápido da operação em tempo real.',
        desc: 'Estes cards mostram mensagens, atendimentos em aberto, clientes online e status do WhatsApp. Eles dão uma leitura rápida da operação.',
        icon: 'chart',
        selectors: ['.analytics-panel .metric-grid', '.metric-grid', '.analytics-panel']
      },
      {
        title: 'Gráficos de performance',
        say: 'Aqui eu te ajudo a enxergar evolução, volume e possíveis gargalos.',
        desc: 'Os gráficos mostram o fluxo de atendimentos e a performance do funil, ajudando a identificar volume, evolução e gargalos.',
        icon: 'line',
        selectors: ['.analytics-charts', '.analytics-charts .chart-card', '#pizzaAtendimento', '#funilAtendimento']
      },
      {
        title: 'Últimos atendimentos',
        say: 'Pra fechar, aqui ficam os atendimentos mais recentes para conferência rápida.',
        desc: 'Aqui ficam as movimentações mais recentes do período filtrado. Use esta lista para conferir clientes, status, horário e data.',
        icon: 'list',
        selectors: ['#lastBox', '.analytics-table-section', '#tabelaAtendimentos']
      }
    ]
  };

  const NS = 'zc-robot-tour-dashboard';
  let stepIndex = 0;
  let raf = 0;
  let scrollTimer = 0;
  ...
  function injectCss() {
    ...
    style.textContent = `
      ...
      .zc-robot-tour-shell.is-open{pointer-events:auto;}
  ...
  function build() {
    ...
    shell.innerHTML = `
      ...
      <div class="zc-robot-tour-stage" aria-live="polite" data-position="right">
        ...
        <section class="zc-robot-tour-card" role="dialog" aria-modal="false" aria-label="Ajuda guiada do Dashboard">
