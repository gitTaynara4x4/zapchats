(function () {
  ...
  const CFG = {
    "key": "atendimentos",
    "page": "Atendimento",
    "title": "Entenda a tela Atendimento",
    "intro": "Vou te mostrar como receber conversas, filtrar atendimentos e responder clientes com segurança.",
    "robotName": "Zappy",
    "robotSrc": "/frontend/img/help/zc-robot-floating.png",
    "storageKey": "zaps_help_atendimentos_robot_tour_seen_v1",
    "steps": [
        {
            "title": "Lista de conversas",
            "say": "Aqui ficam os atendimentos que chegaram para a equipe acompanhar.",
            "desc": "A lateral mostra conversas, clientes e status. Clique em uma conversa para abrir o histórico e responder pelo WhatsApp.",
            "icon": "chat",
            "selectors": [
                "#lista-clientes",
                "aside.flex.flex-col",
        ...
        {
            "title": "Instância, busca e filtros",
            "say": "Use essa parte para escolher o número, pesquisar clientes e separar conversas.",
            "desc": "Você pode trocar a instância de WhatsApp, pesquisar por nome ou telefone e usar filtros como Tudo, Não lidas, Novas e Grupos.",
            "icon": "filter",
            "selectors": [
                "#zc-inst-trigger",
                ".wpp-header-search-row",
                "#wpp-header-search",
                ".wpp-header-filtros-row"
            ]
        },
        {
            "title": "Nova conversa",
            "say": "Quando precisar iniciar um atendimento manual, comece por esse botão.",
            "desc": "O botão de nova conversa abre o fluxo para iniciar contato com um cliente sem esperar ele mandar mensagem primeiro.",
            "icon": "userPlus",
            "selectors": [
                "#btn-sidemodal-nova-conversa",
                ".wpp-header-icons",
                ".zc-topbar-head"
            ]
        },
        {
            ...
            "say": "Depois que escolher um cliente, o histórico aparece nessa área central.",
            "desc": "Aqui você acompanha as mensagens trocadas, carregamento da conversa e a tela inicial quando nenhum atendimento está selecionado.",
            "icon": "eye",
            "selectors": [
                "#chat-main",
                "#historico",
        ...
        },
        {
            ...
            "say": "No topo da conversa ficam as ações importantes daquele cliente.",
            "desc": "Quando houver conversa aberta, use essa área para ver notas do cliente, transferir departamento e acompanhar informações rápidas do atendimento.",
            "icon": "users",
            "selectors": [
                "#chat-header",
                "#btn-sobre",
        ...
        {
            "title": "Responder e anexar",
            "say": "É aqui que o atendente escreve, envia e anexa arquivos para o cliente.",
            "desc": "O rodapé da conversa concentra campo de mensagem, anexos, emojis, áudio e envio. Se a conversa exigir aceite, a barra de atendimento aparece acima do campo.",
            "icon": "send",
            "selectors": [
                "#chat-footer",
                "#wa-composer",
  ...
  const NS = 'zc-robot-tour-atendimentos';
  let stepIndex = 0;
  let raf = 0;
  let scrollTimer = 0;
  ...
  function icon(name) {
    ...
    const icons = {
      ...
      send: `<svg ${common}><path d="M22 2 11 13"></path><path d="m22 2-7 20-4-9-9-4 20-7Z"></path></svg>`,
  ...
  function injectCss() {
    ...
    style.textContent = `
      ...
      body.zc-atendimentos-page .zc-robot-tour-open-btn{
        right:22px;
        bottom:86px;
      }
  ...
  function build() {
    ...
    shell.innerHTML = `
      ...
      <div class="zc-robot-tour-stage" aria-live="polite" data-position="right">
        ...
        <section class="zc-robot-tour-card" role="dialog" aria-modal="false" aria-label="Ajuda guiada de Atendimento">
    ...
    document.addEventListener('click', function (ev) {
      ...
      if (!link) return;
      if (link.closest('aside, nav, .sidebar, .app-sidebar, .main-sidebar, #sidebar, #app-sidebar, #sidebar-atendimentos-slot')) {
        closeTour();
      }
    }, true);
