(function () {
  ...
  const CFG = {
    ...
    intro: 'Vou te mostrar os principais pontos desta tela para você organizar e encontrar contatos com mais segurança.',
    robotName: 'Zappy',
    robotSrc: '/frontend/img/help/zc-robot-floating.png',
    storageKey: 'zaps_help_clientes_robot_tour_seen_v3',
    steps: [
      {
        title: 'Resumo da página',
        say: 'Oi! Aqui você confirma que está na área de clientes e vê o total cadastrado.',
        desc: 'O topo identifica a página e mostra rapidamente a quantidade de clientes carregados no sistema.',
        icon: 'users',
        selectors: ['.section-title', '.section-title h1', '#totalClientes']
      },
      {
        title: 'Busca e filtros',
        say: 'Use essa parte para encontrar clientes sem se perder na lista.',
        desc: 'Pesquise por nome, telefone ou tag e refine a visualização por departamento, responsável e período.',
        icon: 'filter',
        selectors: ['.filters', '#busca', '#select-depto', '#select-resp', '#btn-filtrar']
      },
      {
        title: 'Novo cliente',
        say: 'Quando precisar cadastrar alguém manualmente, comece por este botão.',
        desc: 'O botão Novo cliente abre o cadastro completo, com dados básicos, departamento, responsável, documentos, WhatsApp e endereço.',
        icon: 'userPlus',
        selectors: ['#btn-add-cliente', '.actions-right']
      },
      {
        title: 'Importar e exportar',
        say: 'Aqui ficam as ações para movimentar listas de clientes.',
        desc: 'Use Exportar para baixar dados e Importar para enviar listas em CSV, XLSX ou TXT quando precisar cadastrar em massa.',
        icon: 'file',
        selectors: ['.actions-right', '#btnExportar', '#btnImportar']
      },
      {
        title: 'Tabela de clientes',
        say: 'A lista é onde você confere, seleciona e acessa as ações de cada cliente.',
        desc: 'Na tabela você visualiza nome, telefone, departamento, responsável, data de cadastro e ações disponíveis para cada registro.',
        icon: 'list',
        selectors: ['.box', '.table-wrap', '#corpo-tabela-clientes', '#checkAll']
      }
    ]
  };

  const NS = 'zc-robot-tour-clientes';
  let stepIndex = 0;
  let raf = 0;
  let scrollTimer = 0;
  ...
  function build() {
    ...
    shell.innerHTML = `
      ...
      <div class="zc-robot-tour-stage" aria-live="polite" data-position="right">
        ...
        <section class="zc-robot-tour-card" role="dialog" aria-modal="false" aria-label="Ajuda guiada de Clientes">
