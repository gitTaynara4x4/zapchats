...
def listar_conversas(
    ...
    try:
        ...
        if _has_model_pinned(db):
            pins_user = _pins_get_cached(
                db,
                int(empresa_id),
                user_id,
                int(resolved_inst_id) if resolved_inst_id is not None else None,
            )
        else:
        ...
        if cached:
            ...
            for it in cached.get("items", []) or []:
                ...
                item = dict(it)
                inst_final = iid or _to_int(item.get("instancia_id")) or resolved_inst_id
                conv_key = _conversation_key_cliente(int(cid), int(inst_final or 0))

                item["id"] = conv_key
                item["conversation_id"] = conv_key
                item["conversation_key"] = conv_key
                item["cliente_id"] = int(cid)
                item["cliente_base_id"] = int(cid)
                item["instancia_id"] = int(inst_final) if inst_final else item.get("instancia_id")
                item["is_group"] = False
                item["pinned"] = int(cid) in pins_user
                item["avatar_url"] = _public_avatar_url(int(cid), item.get("avatar_url"))

                items.append(item)

            _recalcular_unread_para_items_cache(
                db,
                empresa_id=int(empresa_id),
                items=items,
        ...
        # Conversa agora é cliente + instância.
        # Antes agrupava só por cliente_id e isso quebrava multi-instância.
        subq = (
            db.query(
            ...
            .filter(*filtros)
            .group_by(m.cliente_id, m.instancia_id)
            .subquery()
        )

        base = db.query(
            subq.c.cid.label("cliente_id"),
            subq.c.iid.label("instancia_id"),
        ...
        pairs_map: Dict[Tuple[int, int], int] = {}

        for cid, iid, mid in normal_pairs:
            if cid is None or iid is None or mid is None:
                continue
            pairs_map[(int(cid), int(iid))] = int(mid)

        if not cursor and pins_user:
            pinned_rows = (
                db.query(
                    m.cliente_id.label("cid"),
                    m.instancia_id.label("iid"),
                ...
                .filter(*filtros)
                .filter(m.cliente_id.in_([int(x) for x in pins_user]))
                .group_by(m.cliente_id, m.instancia_id)
                .order_by(func.max(m.id).desc())
                .all()
            )

            for cid, iid, mid in pinned_rows:
                if cid is None or iid is None or mid is None:
                    continue
                pairs_map[(int(cid), int(iid))] = int(mid)
        ...
        cliente_ids = sorted({cid for cid, _iid in pairs_map.keys()})
        msg_ids = list(pairs_map.values())
        ...
        novas_map: Dict[Tuple[int, int], int] = {}
        if cliente_ids:
            q_novas = (
                db.query(m.cliente_id, m.instancia_id, func.count(m.id))
                .filter(
            ...
            for cid, iid, cnt in q_novas.group_by(m.cliente_id, m.instancia_id).all():
                novas_map[(int(cid), int(iid or 0))] = int(cnt or 0)

        deleted_set = set(_cache_get_json(_k_deleted(empresa_id)) or [])

        items = []
        for (cid, iid), mid in pairs_map.items():
            if int(cid) in deleted_set:
                continue
            ...
            msg_inst_id = int(getattr(msg, "instancia_id", None) or iid)
            conv_key = _conversation_key_cliente(int(c.id), msg_inst_id)
            ts_iso = msg.timestamp.isoformat() if msg.timestamp else None
            novas = int(novas_map.get((int(c.id), msg_inst_id), 0))
            ...
            items.append(
                {
                    ...
                    "nao_lidas": novas,
                    "last_tipo": msg.tipo,
                    "ultima_tipo": msg.tipo,
                    "last_ack": int(getattr(msg, "ack", 0)) if msg.tipo == "saida" else None,
                    "ultima_ack": int(getattr(msg, "ack", 0)) if msg.tipo == "saida" else 0,
                    "instancia_id": msg_inst_id,
                    "instance_name": resolved_inst_name,
                    "pinned": int(cid) in pins_user,
                    "fixado": int(cid) in pins_user,
                }
            )

        items_p = sorted(
            [it for it in items if it.get("pinned")],
            key=lambda it: int(it.get("ultima_msg_id") or 0),
            reverse=True,
        )
        items_o = sorted(
        ...
        next_cursor = int(normal_pairs[-1][2]) if normal_pairs else None
        ...
        raise HTTPException(status_code=500, detail=f"Falha ao listar conversas: {str(e)}")
