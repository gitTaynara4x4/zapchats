...
def _assert_empresa_access(empresa_id: int, user) -> int:
    """
    Garante que o usuário só mexa nas configs da própria empresa.
    """
...
def get_config(
    ...
    # trava por empresa: não deixa pedir config de outra
    _assert_empresa_access(empresa_id, user)

    cfg = (
        db.query(models.EmpresaConfig)
        .filter(models.EmpresaConfig.empresa_id == empresa_id)
        .first()
    )
...
def put_config(
    ...
    # trava por empresa: não deixa salvar em outra
    _assert_empresa_access(empresa_id, user)
