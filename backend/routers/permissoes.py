...
PERMISSOES_CATALOGO = [
    ...
    # ===== NOVAS PERMISSÕES DO MÓDULO DE E-MAIL =====
...
def listar_permissoes(empresa_id: Optional[int] = None, user=Depends(get_current_user)):
...
def syncar_catalogo(user=Depends(get_current_user), db: Session = Depends(get_db)):
...
def salvar_permissoes_do_colaborador(
    ...
    _sync_catalog_to_db(db)
    with db.begin():
        db.execute(text("DELETE FROM colaboradores_permissoes WHERE colaborador_id = :cid"), {"cid": colab_id})
        for pid in ids:
            db.execute(
                text("""
