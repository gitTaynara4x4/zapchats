# backend/integrations/evolution/handlers/messages_update.py
...
def _resolve_empresa_id_by_instance_name(db, instance_name: str | None) -> int | None:
    raw = str(instance_name or "").strip()
    if not raw:
        return None

    row = (
        db.query(models.EmpresaInstancia.empresa_id)
...
async def on_messages_update(first: str, payload: dict | list):
    ...
    with SessionLocal() as db:
        try:
            ...
            if inst_name:
                emp_id = _resolve_empresa_id_by_instance_name(db, inst_name)
    ...
    for p in params:
        try:
            ...
            print(f"[ACK DEBUG] Falha ao emitir ACK por WS msg_id={p['msg_id']}: {e}")
