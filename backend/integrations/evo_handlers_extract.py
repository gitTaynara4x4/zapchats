# backend/integrations/evo_handlers_extract.py
...
def extract_text_from_baileys(obj: dict) -> str:
    ...
    # LISTA (prioriza selectedRowId)
    if "listResponseMessage" in m:
        lrm = m.get("listResponseMessage") or {}
        ss = lrm.get("singleSelectReply") or {}
        sel = ss.get("selectedRowId") or lrm.get("selectedRowId") or lrm.get("title")
        return sel or "[Lista]"
...
def extract_messages_any_shape(data) -> list[dict]:
    """
    Extrai lista de mensagens de payloads variados.
    Dedup por msg_id (key.id ou id) mantendo a versão mais rica.
    """
...
def extract_chats_any_shape(data) -> list[dict]:
    def looks(d: dict) -> bool:
        ...
        if isinstance(jid, str) and (jid.endswith("@g.us") or jid.endswith("@s.whatsapp.net") or jid.endswith("@lid")):
            return True
        if d.get("isGroup") is True or d.get("subject"):
            return True
        return False
    ...
    if isinstance(data, dict):
        for k in ("chats", "items", "data", "result", "rows", "list", "payload", "store"):
            v = data.get(k)
            if isinstance(v, list):
                return [x for x in v if isinstance(x, dict) and looks(x)]
